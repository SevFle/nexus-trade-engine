...
Import safety
    Importing this module never opens a connection and never imports
    ``redis`` — the ``redis`` / ``redis.asyncio`` imports are deferred to
    :func:`make_async_client` / :func:`make_sync_client` call time so
    modules that only need the pure helpers (:func:`is_fresh`,
    :func:`write_heartbeat`) do not pull ``redis-py`` (a transitive dep via
    ``taskiq-redis``) into their import graph. The writing side is driven
    by :func:`heartbeat_loop` — spawned from the taskiq ``WORKER_STARTUP``
    hook via :func:`start_heartbeat` — and the reading side is exposed as
    ``python -m engine.tasks.heartbeat check`` plus the pure
    :func:`is_fresh` helper, so it is unit-testable without a broker.
"""
...
#: Per-command socket read timeout (seconds) for the healthcheck clients.
#: Bounds how long a command blocks waiting on a reply. Kept tight (2s, as
#: originally specified) so an unreachable Valkey cannot wedge either the
#: async writer or the sync ``check`` probe.
DEFAULT_SOCKET_TIMEOUT = 2.0
#: TCP connect timeout (seconds) for the healthcheck clients. Bounds how
#: long a client blocks establishing a connection to Valkey.
...
def make_async_client(url: str | None = None, **kwargs):  # type: ignore[no-untyped-def]
    ...
    ``redis.asyncio`` is imported here (not at module top level) so importing
    :mod:`engine.tasks.heartbeat` never pulls ``redis-py`` into scope — the
    pure helpers stay import-safe for callers that never touch a broker.
    Callers that want bounded command/connect latencies (the production
    heartbeat loop) pass ``socket_timeout`` / ``socket_connect_timeout`` via
    ``**kwargs``; see :func:`start_heartbeat` and :func:`_run_forever`.
    """
...
def make_sync_client(url: str | None = None, **kwargs):  # type: ignore[no-untyped-def]
    ...
    ``redis`` is imported here (not at module top level) so importing
    :mod:`engine.tasks.heartbeat` never pulls ``redis-py`` into scope — the
    pure helpers stay import-safe for callers that never touch a broker.
    """
    import redis
...
async def heartbeat_loop(
    ...
    *,
    ...
    The actual write is delegated to :func:`write_heartbeat` so there is a
    single code path that formats and persists the timestamp — this module
    must never format/``set`` the value in two places.
    """
    while True:
        try:
            await write_heartbeat(client, key=key, clock=clock)
...
def check_once(
    *,
    ...
    When no ``client`` is supplied a fresh sync client is built with
    bounded :data:`DEFAULT_SOCKET_TIMEOUT` / :data:`DEFAULT_SOCKET_CONNECT_TIMEOUT`
    so an unreachable Valkey cannot hang the healthcheck forever.
    """
...
def start_heartbeat(
    *,
    ...
    When no ``client`` is supplied an async client is built with bounded
    :data:`DEFAULT_SOCKET_TIMEOUT` / :data:`DEFAULT_SOCKET_CONNECT_TIMEOUT`
    so an unreachable Valkey cannot wedge the writer's event loop forever.
    """
...
async def stop_heartbeat() -> None:
    ...
    Only :class:`asyncio.CancelledError` from awaiting the cancelled task is
    swallowed — that is the expected, benign unwind path. Any *other*
    exception surfacing from the task is logged at warning level (rather
    than silently suppressed) so a genuinely failing heartbeat is visible.
    Client-close errors are still suppressed because they must not mask an
    ...
    if task is not None and not task.done():
        ...
        try:
            ...
            logger.warning(
                "tasks.heartbeat.stop_unexpected_error",
                error=str(exc),
                error_type=type(exc).__name__,
            )
...
async def _run_forever(*, interval: float) -> None:
    ...
    Builds the async client with bounded
    :data:`DEFAULT_SOCKET_TIMEOUT` / :data:`DEFAULT_SOCKET_CONNECT_TIMEOUT`
    so an unreachable Valkey cannot hang the foreground writer forever.
    """
    client = make_async_client(
        socket_timeout=DEFAULT_SOCKET_TIMEOUT,
        socket_connect_timeout=DEFAULT_SOCKET_CONNECT_TIMEOUT,
