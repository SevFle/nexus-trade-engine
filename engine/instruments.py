...
class AssetType(StrEnum):
        ...
        * any :class:`str` — matched **case-insensitively** and after
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                msg = (
                    f"unknown asset type {value!r}; "
                    f"expected one of: {valid}"
                )
                raise ValueError(msg) from None
        msg = (
            f"cannot parse AssetType from {type(value).__name__}; "
            f"expected str or AssetType"
        )
        raise TypeError(msg)
    ...
    def from_instrument(cls, instrument: Any) -> AssetType:
        ...
        1. If the object exposes an ``asset_type`` attribute that parses
           to a valid :class:`AssetType`, return it. This is the canonical
           path for :class:`engine.core.instruments.Instrument` instances
           created after the ``asset_type`` field was introduced.
