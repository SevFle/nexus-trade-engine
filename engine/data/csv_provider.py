    ...
    * existing ``Date``/``Datetime`` columns are normalised to ``Datetime``.
    ...
    def _coerce_timestamp(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if _is_numeric(dtype):
            series = df["timestamp"]
            self._raise_on_non_finite_epoch(series)
            unit = self._resolve_unit(series)
    ...
    def _normalise_unit(unit: str | None) -> str:
        ...
        if norm not in TIMESTAMP_UNITS:
            raise ValueError(
                f"Invalid timestamp_unit {unit!r}; expected one of {sorted(TIMESTAMP_UNITS)}.",
            )
        return norm
    ...
    def _raise_on_null_timestamps(
        ...
        *original* (pre-parse) frame so callers can fix the source data.
        ...
        positions = bad_mask.arg_true().to_list()
    ...
    @staticmethod
    def _raise_on_non_finite_epoch(series: pl.Series) -> None:
        """Raise :class:`DataValidationError` if a float epoch column has NaN/Inf.

        NaN/Infinity survive the epoch -> ``Datetime`` cast as ``null`` or
        nonsensical values, so they must be rejected up-front. Only float-typed
        columns are inspected: integers cannot represent NaN/Inf.
        """
        if not series.dtype.is_float():
            return
        # ``is_finite()`` is False for NaN, Inf *and* null; exclude nulls so an
        # empty cell is never mis-reported as a bad epoch value.
        bad = (~series.is_finite()) & series.is_not_null()
        if not bool(bad.any()):
            return
        positions = bad.arg_true().to_list()
        raw_values = series.gather(positions).to_list()
        offenders = list(zip(positions, raw_values, strict=True))
        preview_limit = 10
        preview = ", ".join(f"row {i}={v!r}" for i, v in offenders[:preview_limit])
        suffix = (
        ...
        raise DataValidationError(
            f"Column 'timestamp' contains {len(positions)} non-finite "
            f"(NaN/Inf) value(s) that cannot be converted to a datetime. "
            f"Offending values: {preview}{suffix}",
        )
...
def _epoch_to_datetime(col: pl.Expr, unit: str) -> pl.Expr:
    ...
    Multiplication by the unit ``factor`` happens *before* the integer cast and
    is followed by ``.round()``, so fractional epoch values (e.g.
    ``1735689600.5`` seconds) retain their sub-second precision instead of
    being silently truncated by an early ``cast(pl.Int64)``. Integer columns
    are unaffected (the multiply is exact and ``round`` is a no-op).
    """
    if unit in ("s", "ms", "us"):
        factor = {"s": 1_000_000, "ms": 1_000, "us": 1}[unit]
        us = (col * factor).round().cast(pl.Int64)
    elif unit == "ns":
