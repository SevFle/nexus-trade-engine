...
class FilesystemIsolation:
    ...
    def _check_absolute_path_policy(self, path: str) -> None:
        ...
        if resolved == work_dir_real or resolved.startswith(
            work_dir_real + _os_module.sep,
        ):
            return
        for p in self._policy.read_only_paths:
            rp = _orig_realpath(p)
    ...
    def _restricted_open(
        ...
        **kwargs: Any,
        ...
        self._audit_log(
            "open" if not is_write else "open_write",
            str(file),
        )
