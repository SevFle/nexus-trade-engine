...
``subprocess``, ``socket``, ``ctypes``, …).  In addition — as defence-in-depth
mirroring :meth:`RestrictedImporter._is_allowed` — when an allowlist
(:data:`ALLOWED_MODULES`) is configured, any module whose *root* is not on the
allowlist is also rejected, so a strategy smuggling an unlisted module is
caught at parse time rather than only at runtime import time.  The denylist
always takes priority over the allowlist.
"""
...
# :data:`~engine.plugins.allowlist.DENYLIST_MODULES`.  ``allowed`` defaults to
# the frozen :data:`~engine.plugins.allowlist.FROZEN_ALLOWED_MODULES`.
...
class AstImportValidator(ast.NodeVisitor):
    ...
    A module is rejected when its *root* is on the denylist
    (:attr:`forbidden_modules`) **or** — as defence-in-depth — when an
    allowlist (:attr:`allowed`) is configured and the root is not on it.  The
    denylist always wins, identical to the precedence in
    :meth:`RestrictedImporter._is_allowed`.
    ...
    def __init__(
        ...
        if forbidden_modules is not None:
            self.forbidden_modules: frozenset[str] = frozenset(forbidden_modules)
        else:
            self.forbidden_modules = DEFAULT_FORBIDDEN_MODULES

        if forbidden_calls is not None:
            self.forbidden_calls: frozenset[str] = frozenset(forbidden_calls)
        else:
    ...
    def _is_forbidden_module(self, module: str) -> bool:
        """``True`` iff *module*'s root is forbidden by policy.
        ...
        As defence-in-depth, when an allowlist (:attr:`allowed`) is configured
        and non-empty, any module whose root is *not* on the allowlist is also
        rejected.  This keeps the static Layer-1 gate consistent with the
        runtime Layer-1 gate, so a strategy smuggling an unlisted module is
        caught at parse time rather than only at import time.  When
        :attr:`allowed` is empty the allowlist check is a no-op and only the
        denylist applies.
        """
        root = self._root(module)
        if root in self.forbidden_modules:
            return True
        return bool(self.allowed and root not in self.allowed)
...
def validate_strategy_source(
    ...
    """
    validator = AstImportValidator(forbidden_modules, forbidden_calls)
