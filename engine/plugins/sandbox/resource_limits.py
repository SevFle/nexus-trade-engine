...
2. **Memory cap** — enforced via :mod:`tracemalloc`.  The context manager
   snapshots the peak memory high-water mark on entry and, on exit, compares
   the observed peak against ``max_memory_mb``.  A background daemon thread
   additionally polls the *current* allocation while the body runs so a
   sustained breach is detected within ``poll_interval`` seconds; the
   post-hoc peak check then raises :class:`SandboxResourceError`
   deterministically.  (The host sandbox additionally installs an
   ``RLIMIT_AS`` hard cap via :func:`resource.setrlimit` as a kernel-level
   backstop that aborts an over-the-limit allocation in real time.)
...
class ResourceLimits:
    ...
    max_memory_mb:
        ...
        observed by :mod:`tracemalloc`) before a :class:`SandboxResourceError`
        (``kind="memory"``) is raised.  A value ``<= 0`` disables the memory
        guard.
    """
...
def _setup_memory_guard(
    ...
    def _monitor() -> None:
        ...
        # even if the peak were somehow missed.
...
def _finalize_memory(
    ...
    by the tracemalloc peak exceeding ``max_memory_mb`` at exit.
    """
    try:
...
def resource_limits(
    ...
    *,
    ...
    On entry the SIGALRM CPU timer is armed (when usable) and ``tracemalloc``
    peak tracing is (re)started; on exit the timer is cancelled, the prior
    signal handler is restored, the memory peak is compared against the cap,
    and any breach raises :class:`SandboxResourceError`.
    ...
    SandboxResourceError:
        ...
        observed peak or an active breach flag indicates a violation).
