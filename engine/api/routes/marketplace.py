...
# ---------------------------------------------------------------------------
# Ratings request / response models
# ---------------------------------------------------------------------------
...
class RatingRequest(BaseModel):
    ...
    review: str | None = Field(
        default=None,
        max_length=2000,
        description="Optional free-text review (<= 2000 chars).",
    )
...
async def submit_rating(
    ...
    One rating per ``(strategy_id, user)`` pair — resubmitting performs an
    upsert that preserves the original ``created_at`` timestamp. Returns
    ``201 Created`` on success and ``400 Bad Request`` on validation failure.
    """
    try:
...
async def get_ratings(
    ...
    """Return aggregate rating stats plus a page of recent reviews.

    The aggregate covers *every* submitted rating; the ``reviews`` list only
    includes records carrying non-empty review text. Returns ``400 Bad
    Request`` if the store rejects the pagination arguments.
    """
    try:
