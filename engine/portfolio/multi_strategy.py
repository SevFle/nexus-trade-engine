...
class PortfolioEvaluation:
    ...
    def is_noop(self) -> bool:
        ...
        return not self.signals or self.total_capital == 0
...
def _finite(value: float, label: str) -> float:
    ...
    Strings (even numeric ones like ``"1000"``) are rejected outright so a
    caller cannot smuggle in a value that ``float()`` would silently parse
    into a number with surprising semantics; only genuine ``int``/``float``
    values are accepted. ``None`` and other non-numeric types fall through to
...
class MultiStrategyPortfolio:
    ...
    async def evaluate_all(
        ...
        *,
        ...
        # No-op short-circuits before any strategy runs:
        #   - an empty registry has nothing to evaluate; and
        #   - with zero total capital nothing can be deployed, so every
        #     signal would net to a zero-dollar HOLD anyway.
        # Either way we skip strategy evaluation and return a clean,
        # empty result with consistent bookkeeping (notably a
        # ``capital_utilization`` of 0.0 rather than a divide-by-zero).
        if not self._strategies or self._total_capital == 0:
        ...
        for sid, strategy in list(self._strategies.items()):
            ...
            if inspect.isawaitable(raw):
                try:
                    ...
                    # ``wait_for`` raises the builtin ``TimeoutError`` only
                    # when its own deadline expires, so this branch is a
                    # genuine per-strategy timeout (the sync frame's own
                    # ``TimeoutError`` was already handled above).
                    logger.warning(
                    ...
                    # coroutine (RuntimeError, ValueError, …) is a strategy
                    # failure, not a deadline expiry — record it the same
                    # way as the synchronous path so one bad strategy can
