...
* **Zero-capital is a valid no-op.** When total current value is ``0``
  ...
  zero-capital short-circuit. (Note this is about *current* values, not
  target weights: an all-zero *target* policy is rejected at construction.)
...
class PortfolioRebalancerError(ValueError):
    ...
    weight, value, or threshold, an empty target allocation, an all-zero
    target allocation, or a non-dict mapping argument."""
...
class PortfolioRebalancer:
    ...
    target_weights:
        ...
        sum to 1.0 (``{"a": 1, "b": 1}`` ≡ ``{"a": 0.5, "b": 0.5}``). At
        least one weight must be strictly positive — an all-zero target
        policy is meaningless and is rejected.
    current_values:
        ...
        exit) and vice-versa (they will be targeted for entry). An all-zero
        / empty current state is a valid no-op (nothing to rebalance).
    threshold:
    ...
    PortfolioRebalancerError
        ...
        ``target_weights``, an all-zero ``target_weights``, or a non-dict
        mapping argument.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Per-weight validation guarantees each value is finite and
        # non-negative, but it does NOT guarantee at least one is positive.
        # An all-zero target policy (e.g. {"a": 0.0, "b": 0.0}) is
        # meaningless — there is no allocation to hold or drift toward — so
        # reject it explicitly with a clear message. The alternatives all
        # hide a misconfiguration: silently dividing by zero would yield
        # NaN weights that slip past downstream ``isfinite`` checks, and
        # papering over it with arbitrary equal shares would invent an
        # allocation the operator never asked for.
        target_sum = sum(targets.values())
        if target_sum <= 0.0:
            raise PortfolioRebalancerError(
                "target_weights must contain at least one non-zero value; "
                f"all {len(targets)} weight(s) are zero"
            )
