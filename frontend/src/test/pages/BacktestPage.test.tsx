...
describe("BacktestPage", () => {
  ...
  it("dedupes strategy options that collapse to the same submitted value", async () => {
    // Two bare-string entries resolve to the same id/label "dup"; an entry
    // missing an id falls back to its label as the submitted value too.
    listStrategies.mockResolvedValue({
      strategies: ["dup", "dup", { name: "Momentum Alpha" }],
    });
    renderPage();
    await waitFor(() => {
      expect(screen.getByRole("option", { name: "dup" })).toBeInTheDocument();
    });
    // Only one "dup" option survives dedup-by-value.
  ...
  it("rejects a zero initial capital as invalid", async () => {
    listStrategies.mockResolvedValue(STRATEGIES);
    renderPage();
    await waitFor(() => {
      expect(
    ...
    // Fill the rest of the form so only the capital trips validation.
    fireEvent.change(screen.getByTestId("backtest-strategy-select"), {
      target: { value: "momentum-alpha" },
    });
    fireEvent.change(screen.getByTestId("backtest-symbol-input"), {
    ...
    // Zero in several shapes must all be rejected as invalid without ever
    // parsing the value into a float (which would lose precision).
    for (const zero of ["0", "0.0", "0.00", "00", "00.00"]) {
      fireEvent.change(screen.getByTestId("backtest-capital-input"), {
        target: { value: zero },
      });
      fireEvent.click(screen.getByTestId("backtest-submit"));
      expect(
        screen.getByTestId("initial_capital-error"),
      ).toBeInTheDocument();
    }
    expect(runBacktest).not.toHaveBeenCalled();

    // A genuinely positive value clears the error and submits.
    fireEvent.change(screen.getByTestId("backtest-capital-input"), {
      target: { value: "0.01" },
    });
    fireEvent.click(screen.getByTestId("backtest-submit"));
    await waitFor(() => expect(runBacktest).toHaveBeenCalled());
    expect(runBacktest.mock.calls[0][0].initial_capital).toBe("0.01");
  });
