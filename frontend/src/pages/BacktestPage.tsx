 ...
 *
 * The returned {@link StrategyOption.id} is the *real* strategy id when
 * present, otherwise an empty string. Callers must derive React keys from
 * the label plus position and submit ``id || label`` — never a synthesized
 * key, which would leak a non-existent identifier to the backend.
 */
...
function validate(form: BacktestForm): ValidationErrors {
  ...
  // The regex guarantees a well-formed non-negative decimal. The trailing
  // ``[1-9]`` test rejects zero values (``"0"``, ``"0.0"``, ``"00.00"``) so
  // the capital is strictly positive — without ever materializing a float,
  // which would lose precision for large/notional capitals (a naive
  // ``Number(...) <= 0`` guard would re-introduce that loss).
  if (
    !/^\d+(\.\d+)?$/.test(form.initial_capital) ||
    !/[1-9]/.test(form.initial_capital)
  ) {
...
export default function BacktestPage() {
  ...
  const options = useMemo<StrategyOption[]>(() => {
    // Dedupe by the value the <option> submits (``id || label``). Two list
    // entries can collapse to the same submitted value — e.g. bare-string
    // ids that duplicate an object's name, or legacy entries missing an id
    // that fall back to their label — which would render duplicate <option
    // value=...> siblings and make the selection ambiguous. Keep the first
    // occurrence in label order.
    const seen = new Set<string>();
    return (strategiesData?.strategies ?? [])
      .map(toStrategyOption)
      // Keep any option that has something to submit (id) or display
      // (label); entries with neither are unactionable noise.
      .filter((opt) => opt.id.length > 0 || opt.label.length > 0)
...
function SubmitSuccess({ result, form, options }: SubmitSuccessProps) {
  // The submitted ``strategy_name`` is the option id when present, otherwise
  // the label (see the ``value={opt.id || opt.label}`` rendering). Match on
  // either so the success banner always resolves a human-readable label.
  const strategyLabel =
    options.find(
      (o) => o.id === form.strategy_name || o.label === form.strategy_name,
    )?.label ?? form.strategy_name;
  return (
    <section
      role="status"
      aria-live="polite"
