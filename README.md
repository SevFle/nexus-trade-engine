...
- [~] Live broker integration *(partial — read-only `AlpacaDataProvider`
      for market data is reachable; the order-routing adapters
      `engine/execution/live_backend.py` (`LiveExecutionBackend`) and
      `engine/core/brokers/alpaca/` (`AlpacaTradingClient`) have landed
      but are not mounted on any route, and the live loop is scaffolded)*
