...
Environment consumed (defaults shown):
    ...
    POSTGRES_USER       nexus
    POSTGRES_PASSWORD   REQUIRED (raw, un-encoded password)
    POSTGRES_DB         nexus
...
def build_database_url(
    *, user: str, password: str, db: str, host: str, port: str
    ...
    authenticating, so the original password round-trips exactly.
    """
    return (
        "postgresql+asyncpg://"
...
def build_valkey_url(*, host: str, port: str, password: str = "") -> str:
    ...
    """
    if password:
        return f"valkey://:{quote(password, safe='')}@{host}:{port}/0"
