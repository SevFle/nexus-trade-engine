...
class TestGitHubAuthenticate:
    ...
    async def test_authenticate_happy_path_new_user(self, github_provider, mock_settings):
        ...
        async def _set_active(user):
            user.is_active = True

        mock_db.refresh = AsyncMock(side_effect=_set_active)
    ...
    async def test_authenticate_fallback_name_when_null(
        ...
        async def _set_active2(user):
            user.is_active = True

        mock_db.refresh = AsyncMock(side_effect=_set_active2)
    ...
    async def test_authenticate_fallback_name_when_null_login_missing(
        ...
        async def _set_active3(user):
            user.is_active = True

        mock_db.refresh = AsyncMock(side_effect=_set_active3)
