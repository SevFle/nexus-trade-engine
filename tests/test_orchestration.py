...
Covers the full public surface of
...
* and the **robustness fixes** required by gh#1093:
  ...
  2. ``run_all`` bounds the awaitable with the ``asyncio.timeout()``
     context manager; when the orchestrator's OWN deadline elapses the
     strategy is logged as ``orchestrator.strategy_timeout`` and skipped
     while the remaining strategies still contribute,
  3. a strategy that raises the **builtin** ``TimeoutError`` itself —
     from the synchronous ``evaluate()`` call frame *or* from inside the
     ``await`` while under a cap — is reported as
     ``orchestrator.strategy_failed`` and **never** masquerades as a
     timeout. The disambiguation keys off the context manager's
     ``.expired()`` (a ``timed_out`` sentinel set before the await is
     ``True`` in *both* cases and cannot tell them apart on its own),
...
class TestStrategyTimeout:
    """The ``asyncio.timeout()`` handling in ``run_all``."""
    ...
    async def test_timeout_log_carries_strategy_id_and_cap(self):
        # The structured log must record which strategy timed out and the
        # configured cap so the event is auditable.
        class _Slow:
            id = "slow"
        ...
        with capture_logs() as logs:
            orch = StrategyOrchestrator([_Slow()], _FakeCostModel())
            await orch.run_all({}, timeout_seconds=0.02)

        to_logs = [e for e in logs if e["event"] == "orchestrator.strategy_timeout"]
        assert len(to_logs) == 1
        assert to_logs[0]["strategy_id"] == "slow"
        assert to_logs[0]["timeout"] == 0.02
    ...
    async def test_async_strategy_raising_builtin_timeout_without_cap_is_failure(self):
        ...
        # run WITHOUT a cap, propagates through the generic ``await raw``
        # path and is reported as strategy_failed. With no cap the
        # ``asyncio.timeout()`` guard is never entered, so the builtin
        # TimeoutError cannot be mistaken for a timeout.
    ...
    async def test_sync_strategy_raising_builtin_timeout_is_failure_not_timeout(self):
        # A *sync* evaluate that raises the builtin TimeoutError (from the
        # synchronous call frame, which is outside the asyncio.timeout()
        # context) must be reported as strategy_failed, never masquerade
        # as a timeout.
        with capture_logs() as logs:
            orch = StrategyOrchestrator(
                [
...
class TestTimeoutDisambiguation:
    """The heart of the asyncio.timeout() fix.

    A strategy that raises its OWN ``TimeoutError`` while running UNDER
    the orchestrator's timeout cap must be reported as
    ``orchestrator.strategy_failed`` — NOT ``orchestrator.strategy_timeout``.

    This is the one case a naive ``timed_out = True`` sentinel set before
    the await gets wrong (it is True whether the deadline fired or the
    strategy raised). The orchestrator disambiguates via the context
    manager's ``.expired()``.
    """

    async def test_async_strategy_own_timeout_under_cap_is_failure(self):
        # The strategy raises a TimeoutError immediately, but a generous
        # cap is in effect — so the orchestrator's own deadline clearly
        # did NOT fire.
        class _OwnTimeout:
            id = "own"

            async def evaluate(self, md, cm):
                # Mimic e.g. an inner asyncio.wait_for / HTTP socket timeout
                # surfacing from the strategy body itself.
                raise TimeoutError("inner httpx read timeout")
        ...
        with capture_logs() as logs:
            ...
            # Generous cap: the orchestrator's deadline cannot be the cause.
            raw = await orch.run_all({}, timeout_seconds=5.0)

        # It is a FAILURE (strategy raised its own TimeoutError), never a
        # genuine orchestrator timeout.
        assert any(
            e["event"] == "orchestrator.strategy_failed" and e["strategy_id"] == "own"
            for e in logs
        )
        assert not any(e["event"] == "orchestrator.strategy_timeout" for e in logs)
        assert [s.strategy_id for s in raw] == ["good"]
    ...
    async def test_async_strategy_own_timeout_after_work_under_cap_is_failure(self):
        # Same disambiguation, but the strategy does real awaited work for
        # a while before raising its own TimeoutError. This guards against
        # a naive implementation that only checks wall-clock elapsed time:
        # here real time passes (but less than the cap) and it is STILL a
        # failure, because .expired() is the only signal that matters.
        class _OwnTimeoutAfterWork:
            id = "own"

            async def evaluate(self, md, cm):
                await asyncio.sleep(0.02)  # well under the cap
                raise TimeoutError("inner timeout after some work")

        with capture_logs() as logs:
            orch = StrategyOrchestrator(
                [_OwnTimeoutAfterWork()],
                _FakeCostModel(),
            )
            raw = await orch.run_all({}, timeout_seconds=1.0)
    ...
    async def test_real_timeout_and_own_timeout_coexist_in_one_cycle(self):
        # One strategy genuinely exceeds the cap → strategy_timeout.
        # Another raises its own TimeoutError well under the cap →
        # strategy_failed. Both are classified correctly in a single cycle.
        class _Slow:
            id = "slow"
        ...
        with capture_logs() as logs:
            orch = StrategyOrchestrator(
                [_Slow(), _OwnTimeout(), _AsyncStrategy("good", [_sig("AAPL", Side.BUY, "good")])],
                _FakeCostModel(),
            )
            raw = await orch.run_all({}, timeout_seconds=0.05)

        events = {(e["event"], e["strategy_id"]) for e in logs}
        assert ("orchestrator.strategy_timeout", "slow") in events
        assert ("orchestrator.strategy_failed", "own") in events
        # No misclassification in either direction.
        assert ("orchestrator.strategy_timeout", "own") not in events
        assert ("orchestrator.strategy_failed", "slow") not in events
        assert [s.strategy_id for s in raw] == ["good"]

    async def test_zero_timeout_fires_immediately(self):
        # Edge: a zero (or near-zero) cap means the orchestrator's deadline
        # elapses essentially at once → genuine timeout, not a failure.
        class _Slow:
            id = "slow"
        ...
        with capture_logs() as logs:
            await StrategyOrchestrator([_Slow()], _FakeCostModel()).run_all(
                {}, timeout_seconds=0.0
            )
...
class TestNoneMetadataResolution:
    ...
    def test_empty_metadata_resolves_without_error(self):
        # A falsy-but-not-None metadata (empty dict) is also handled: the
        # ``... if template.metadata else {}`` branch yields a fresh dict.
        template = _sig("AAPL", Side.BUY, "x").model_copy(update={"metadata": {}})
        orch = StrategyOrchestrator([_AsyncStrategy("x")], _FakeCostModel())
        out = orch.aggregate_signals([template])
        assert isinstance(out[0].metadata, dict)
        assert out[0].metadata["orchestrator_sources"] == ["x"]
...
class TestMetadataDeepCopyIsolation:
    ...
    async def test_resolved_metadata_is_independent_of_template(self):
        # The resolved signal's metadata must be a distinct object from the
        # template's metadata so a later mutation of either is isolated.
        src = _sig("AAPL", Side.BUY, "x", metadata={"k": 1})
        orch = StrategyOrchestrator([_AsyncStrategy("x")], _FakeCostModel())
        resolved = orch.aggregate_signals([src])[0]

        assert resolved.metadata is not src.metadata
        # Mutating the template after the fact does not affect the resolved.
        src.metadata["k"] = 999
        assert resolved.metadata["k"] == 1
