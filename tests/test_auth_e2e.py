...
# JSONB is a Postgres-only type; SQLite (used by the in-memory E2E DB) needs a
# TEXT fallback so Base.metadata.create_all() can build the schema from the
# real ORM models instead of a hand-rolled Table() definition that drifts
# (this drift was the root cause of the missing ``processing_restricted`` column).
