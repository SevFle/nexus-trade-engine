...
class TestGoogleAuthProviderName:
    def test_name(self, provider):
        assert provider.name == "google"


class TestGoogleGetAuthorizeUrl:
    ...
    def test_url_contains_state(self, provider):
        url = provider.get_authorize_url(state="xyz789")
...
class TestGoogleAuthenticate:
    ...
    async def test_successful_auth_creates_user(self, provider, db_session):
        ...
        mock_token_resp.json.return_value = {"access_token": "g-token"}
        mock_token_resp.raise_for_status = MagicMock()

        profile = {"sub": "google-123", "email": "user@example.com", "name": "Test User"}
        ...
        assert result.user_info.external_id == "google-123"
    ...
    async def test_disabled_user_returns_error(self, provider, db_session):
        ...
        profile = {"sub": "google-999", "email": "disabled@example.com", "name": "Disabled"}
        mock_user_resp = MagicMock()
        mock_user_resp.json.return_value = profile
        mock_user_resp.raise_for_status = MagicMock()
    ...
    async def test_duplicate_email_returns_error(self, provider, db_session):
        ...
        profile = {"sub": "google-new", "email": "taken@example.com", "name": "New User"}
        mock_user_resp = MagicMock()
        mock_user_resp.json.return_value = profile
        mock_user_resp.raise_for_status = MagicMock()
