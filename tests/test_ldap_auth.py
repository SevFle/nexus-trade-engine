...
def _make_mock_ldap_modules(attrs=None, search_results=None, bind_error=None):
    mock_conn = MagicMock()
    if bind_error:
    ...
    mock_ldap_mod.OPT_TIMEOUT = 8

    mock_filter_mod = MagicMock()
    mock_filter_mod.escape_filter_chars.side_effect = lambda x: x

    return {
        "ldap": mock_ldap_mod,
        "ldap.filter": mock_filter_mod,
    }
...
class TestLDAPAuthenticate:
    ...
    async def test_authenticate_bind_failure(self, ldap_provider, mock_settings):
        modules = _make_mock_ldap_modules(
    ...
    async def test_authenticate_user_not_found_in_ldap(self, ldap_provider, mock_settings):
        modules = _make_mock_ldap_modules(search_results=[])
    ...
    async def test_authenticate_existing_user_no_role_change(
        ...
        modules = _make_mock_ldap_modules(attrs=attrs)
    ...
    async def test_authenticate_escapes_username(self, ldap_provider, mock_settings):
        modules = _make_mock_ldap_modules()
        mock_filter = modules["ldap.filter"]
        mock_filter.escape_filter_chars.side_effect = lambda x: x.replace("*", "\\2a")
        ...
        mock_filter.escape_filter_chars.assert_called_once_with("test*user")
