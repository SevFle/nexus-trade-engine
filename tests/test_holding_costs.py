...
class TestHardToBorrow:
    def test_one_day_at_15_pct(self):
        ...
        assert hard_to_borrow_cost(Decimal("100000"), Decimal("0.15")) == Decimal("41.10")

    def test_30_days_scales_linearly(self):
        out = hard_to_borrow_cost(Decimal("100000"), Decimal("0.15"), days=30)
    ...
    def test_extreme_htb_rate(self):
        ...
        out = hard_to_borrow_cost(Decimal("50000"), Decimal("2.0"), days=1)
        assert out == Decimal("273.97")

    def test_zero_short_zero_cost(self):
        assert hard_to_borrow_cost(Decimal("0"), Decimal("0.10")) == Decimal("0.00")

    def test_zero_rate_zero_cost(self):
        assert hard_to_borrow_cost(Decimal("100000"), Decimal("0")) == Decimal("0.00")
    ...
    def test_negative_days_rejected(self):
        with pytest.raises(ValueError):
            hard_to_borrow_cost(Decimal("100"), Decimal("0.10"), days=-1)

    def test_zero_days_per_year_rejected(self):
        with pytest.raises(ValueError):
            hard_to_borrow_cost(Decimal("100"), Decimal("0.10"), days_per_year=0)
...
class TestDividendPayment:
    def test_known_value(self):
        ...
        assert dividend_payment(Decimal("250"), Decimal("0.42")) == Decimal("105.00")

    def test_fractional_shares(self):
        # 100.5 shares * $1.00 = $100.50
        assert dividend_payment(Decimal("100.5"), Decimal("1.00")) == Decimal("100.50")

    def test_zero_shares_zero_dividend(self):
        assert dividend_payment(Decimal("0"), Decimal("1.00")) == Decimal("0.00")

    def test_zero_per_share_zero_dividend(self):
        assert dividend_payment(Decimal("100"), Decimal("0")) == Decimal("0.00")
...
class TestReinvestedShares:
    def test_fractional_default(self):
        ...
        assert reinvested_shares(Decimal("105"), Decimal("50")) == Decimal("2.1000")

    def test_fractional_quantises_to_four_decimals(self):
        # $100 / $33 = 3.0303030303... → 3.0303 (4dp).
        assert reinvested_shares(Decimal("100"), Decimal("33")) == Decimal("3.0303")

    def test_integer_truncation_with_fractional_false(self):
        # $105 / $50 = 2.1 → 2 (integer truncation).
        assert reinvested_shares(Decimal("105"), Decimal("50"), fractional=False) == Decimal("2")

    def test_zero_cash_zero_shares(self):
        assert reinvested_shares(Decimal("0"), Decimal("50")) == Decimal("0.0000")
...
class TestReinvestmentResidualCash:
    def test_integer_truncation_residual(self):
        ...
        assert reinvestment_residual_cash(Decimal("105"), Decimal("50"), Decimal("2")) == Decimal(
            "5.00"
        )
    ...
    def test_overspend_rejected(self):
        ...
        with pytest.raises(ValueError):
            reinvestment_residual_cash(Decimal("100"), Decimal("50"), Decimal("3"))

    def test_negative_inputs_rejected(self):
        with pytest.raises(ValueError):
            reinvestment_residual_cash(Decimal("-1"), Decimal("50"), Decimal("0"))
        with pytest.raises(ValueError):
            reinvestment_residual_cash(Decimal("100"), Decimal("-1"), Decimal("0"))
        with pytest.raises(ValueError):
            reinvestment_residual_cash(Decimal("100"), Decimal("50"), Decimal("-1"))
