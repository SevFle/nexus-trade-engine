...
class TestBaseProviderMapRoles:
    ...
    def test_map_roles_unknown_roles_ignored(self):
        p = self._make_provider()
        # Least-privilege fallback: when no recognized role is supplied
        # the function must return ``viewer`` (the lowest-privilege role)
        # rather than ``user`` which has write privileges.
    ...
    def test_map_roles_empty_list_returns_viewer(self):
        ...
        # list must collapse to ``viewer`` so that a misconfigured IdP
        # that asserts no recognized groups cannot gain write privileges.
