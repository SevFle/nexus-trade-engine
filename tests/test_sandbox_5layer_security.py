...
class TestLayerComposition:
    ...
    def test_violations_from_all_layers_collected_on_deactivate(self) -> None:
        policy = SandboxPolicy.from_trust_level(TrustLevel.UNTRUSTED, plugin_id="multi_violation")
        ctx = SandboxContext(policy)
        ctx.activate()
        try:
            with pytest.raises(ImportError):
...
class TestSecurityEventLogging:
    ...
    def test_context_collects_violations_from_all_layers(self) -> None:
        policy = SandboxPolicy.from_trust_level(TrustLevel.UNTRUSTED, plugin_id="event_test")
        ctx = SandboxContext(policy)
        ctx.activate()
        try:
            with pytest.raises(ImportError):
