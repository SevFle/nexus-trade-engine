...
`WS /api/v1/ws/events`). This page was split out of
[`api-reference.md`](api-reference.md) to keep the HTTP route catalog
focused and under the docs length cap; the REST surface and the shared
auth model (JWT, API keys, legal-acceptance gate) live there. The
trusted-proxy IP-resolution model that governs auth rate-limiting is
documented in [`deployment.md`](deployment.md#websocket).
...
The "per IP" IP is resolved under a **trusted-proxy** model (gh#1497):
the raw peer IP is used unless it is in the **CIDR-aware**
`TRUSTED_PROXIES` allow-list, in which case the rightmost
`X-Forwarded-For`/`X-Real-IP` hop is trusted — see
[deployment → WebSocket](deployment.md#websocket) for the spoofing bypass
the CIDR fix closed.
