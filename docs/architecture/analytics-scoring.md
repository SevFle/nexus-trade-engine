...
The measurement, governance, and tuning layer of the core engine. This
page was split out of [`core-domains.md`](core-domains.md) (which now
covers instruments, orchestration, cost/risk, execution, and accounting)
so each stays under the 500-line doc cap. Read `core-domains.md` first
for the domain map; this page is the deep-dive on the numbers.
...
- [`core-domains.md`](core-domains.md) — the rest of the core domain
  layer (orchestration, cost/risk, execution, accounting) and the
  "wired vs. schema-only" quick reference that spans both pages.
- [`known-limitations.md`](../known-limitations.md) — including the
  status of the 86-KPI `PerformanceReport` (schema landed, no analyzer
  wired) and the live/paper execution story.
