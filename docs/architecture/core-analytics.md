# Core analytics, scoring & governance

This is the analysis-and-governance half of the domain map. Its
companion, [`core-domains.md`](core-domains.md), covers the
**runtime** pipeline — instruments, multi-strategy orchestration,
cost/risk modeling, execution backends, and portfolio accounting. This
document covers what happens *after* a run: performance analytics, the
86-KPI taxonomy, composite strategy scoring, the lifecycle/versioning
services, and parameter optimization.

Everything below lives under [`engine/core/`](../../engine/core/) (and
[`engine/core/analytics/`](../../engine/core/analytics/) for the
86-KPI schema).
...
- [`core-domains.md`](core-domains.md) — the runtime half of the domain
  map (instruments, orchestration, cost/risk, execution, portfolio
  accounting), plus the cross-cutting "Wired vs. schema-only" table.
