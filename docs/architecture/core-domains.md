...
containers (`MetricsReport` vs. the 86-KPI `PerformanceReport`), the
composite strategy scorer, lifecycle/version governance, and the
parameter optimizer — is documented in
[`analytics.md`](analytics.md). It was split out of this file when the
domain map crossed the 500-line documentation cap; the boundary is
clean (runtime-vs-post-trade numerics), so the two files read in either
order.

## See also

- [`analytics.md`](analytics.md) — analytics, scoring, and optimization
  (the post-trade evaluation half of this map).
