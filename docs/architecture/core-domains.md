...
## What lives in the companion doc

This document covers the **decision → execution → accounting** path.
The half of the domain layer that sits *after* a backtest — performance
analytics (`MetricsReport` vs. the 86-KPI `PerformanceReport`), the
composite `strategy_evaluator`, the lifecycle/versioning governance
services, and the `param_optimizer` — moved to its own page to stay
under the 500-line cap:

- **[`analytics-and-governance.md`](analytics-and-governance.md)** —
  the analytics taxonomy, scoring, governance, and optimization. Its
  closing "Wired vs. schema-only" table consolidates which of these
  capabilities are reachable from a route today.
...
- [`ARCHITECTURE.md`](../../ARCHITECTURE.md) — headline components
  ...
  interface.
- [`analytics-and-governance.md`](analytics-and-governance.md) — the
  second half of the capability view: performance analytics, strategy
  scoring & governance, and optimization.
