...
orchestration, the cost/risk and execution model, portfolio accounting,
and post-trade cost analysis. Performance analytics, the 86-KPI taxonomy,
strategy scoring/governance, and parameter optimization live in the
companion [`core-analytics.md`](core-analytics.md).
...
The analysis-and-governance half of the domain map — `MetricsReport`
vs. the 86-KPI `PerformanceReport`, companion analytics modules,
composite strategy scoring (`strategy_evaluator`), lifecycle &
versioning services, and parameter optimization — lives in a
companion file to keep this one under the per-file line cap:
see [`core-analytics.md`](core-analytics.md).
...
- [`core-analytics.md`](core-analytics.md) — the analysis-and-governance
  half of the domain map (performance analytics, 86-KPI `PerformanceReport`,
  composite scoring, lifecycle/versioning, parameter optimization).
