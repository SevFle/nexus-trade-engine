...
|---|---|---|---|
| 0. Static source validation | best-effort in-process | **shipped** | [`ImportValidator`](../../engine/plugins/restricted_importer.py) walks the strategy's AST **before** compile/exec and rejects `import` of a blocked module, plus bare calls to `__import__`/`exec`/`eval`/`compile` and `importlib.import_module(…)`. An exact match in the blocked set can never be overridden by the allowlist. See [ADR-0010](../adr/0010-static-ast-validation-toctou-loading.md). A newer, structured successor — [`ASTValidator`](../../engine/plugins/sandbox/ast_validator.py) — landed in #1647 but is **not yet wired** into the registry/sandbox loader: it returns a typed [`ValidationResult`](../../engine/plugins/sandbox/ast_validator.py) of [`Violation`](../../engine/plugins/sandbox/ast_validator.py) records (line/col + machine-readable `code` + `severity`) instead of a flat `list[str]`, models a three-way **allowlist + denylist + forbidden-calls** policy with **denylist precedence** (a denied root is blocked even if also allowlisted), and additionally flags `from ..` relative imports that escape the strategy package (`level > 1`). It is `validate()`-total (a `SyntaxError` is captured as a violation rather than raised) and is exercised by [`tests/test_ast_validator.py`](../../tests/test_ast_validator.py). Swapping the registry's load-time call to it is a tracked follow-up — same shape as the [`PathValidator`](../../engine/plugins/sandbox/filesystem.py) primitive (landed, not yet wired) documented in layer 4 below. |
...
| [`engine/plugins/restricted_importer.py`](../../engine/plugins/restricted_importer.py) | Allowlist import hook (layer 1) + static `ImportValidator` AST checker (layer 0). This is the load-time gate the registry calls (`ImportValidator(DENYLIST_MODULES).validate(source)`); it returns a flat `list[str]`. |
| [`engine/plugins/sandbox/ast_validator.py`](../../engine/plugins/sandbox/ast_validator.py) | Structured static validator (layer 0). `ASTValidator` returns a typed `ValidationResult` of `Violation` records and models allowlist + denylist + forbidden-calls with denylist precedence. Landed (#1647) and unit-tested, but **not yet wired** into `registry.py` / the sandbox loader — a reusable successor to `ImportValidator`, not a replacement on the request path yet. |
