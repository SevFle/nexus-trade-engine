...
|---|---|---|---|
| 0. Static source validation | best-effort in-process | **shipped** | [`ImportValidator`](../../engine/plugins/restricted_importer.py) walks the strategy's AST **before** compile/exec and rejects `import` of a blocked module, plus bare calls to `__import__`/`exec`/`eval`/`compile` and `importlib.import_module(…)`. An exact match in the blocked set can never be overridden by the allowlist. A stricter successor, [`ASTValidator`](../../engine/plugins/sandbox/ast_validator.py), additionally blocks `from … import *` wildcards and relative imports that escape the strategy package — but it is **library-only and not wired** into the load path, so the wired `ImportValidator` is the parse-time check that actually runs today. See [ADR-0010](../adr/0010-static-ast-validation-toctou-loading.md) and [known-limitations](../known-limitations/auth-and-sandbox-drift.md#ast-validator-two-implementations). |
