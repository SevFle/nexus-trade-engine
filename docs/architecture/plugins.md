...
| 3. Resource limits | best-effort in-process | **shipped (partial)** | `resource.setrlimit` enforces `RLIMIT_AS` (memory, from `manifest.resources.max_memory`) and `RLIMIT_NOFILE` (file descriptors) on Linux. `manifest.resources.max_cpu_seconds` is enforced as an **asyncio wall-clock** `wait_for` timeout that raises plain `TimeoutError` — **not** `RLIMIT_CPU`/`SIGXCPU`. A dedicated `ResourceLimitExceededError` + `CpuTimeLimiter` (SIGXCPU) are scaffolded in [`exceptions.py`](../../engine/plugins/exceptions.py) (#1404) but **unwired**: no `CpuTimeLimiter` class exists in the sandbox, `RLIMIT_CPU` is never set, and the exception is never imported or raised anywhere. |
...
| [`engine/plugins/exceptions.py`](../../engine/plugins/exceptions.py) | `ResourceLimitExceededError` — the intended signal for a strategy blowing a `RLIMIT_CPU`/`SIGXCPU` budget. Defined (#1404) but currently **dead code**: the `CpuTimeLimiter` its docstring references is unimplemented, and the active CPU guard is the asyncio `TimeoutError` in layer 3 above. |
