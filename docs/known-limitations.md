...
vars are already enumerated in `.env.example` under the
`── MCP server (engine/mcp) ──` block, with a header comment pointing
back here — so operator discovery of the knobs is not a gap; only the
runnable transport is.)
...
already in `.env.example`; just verify it stays in sync with
[`config.py`](../engine/mcp/config.py) on future edits. The PLR0911
