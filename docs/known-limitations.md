...
> Auth/sandbox and real-time surfaces (the LDAP / OIDC / sandbox-AST
> "two implementations" splits and the two WebSocket gaps) live in the
> companion page,
> [`auth-and-sandbox-drift.md`](known-limitations/auth-and-sandbox-drift.md),
> to keep this file under the 500-line doc limit. They are all P1/P2
> and share one shape: a stricter reference implementation exists on
> disk but is not wired into the runtime path.
...
(see [LDAP has no route](known-limitations/auth-and-sandbox-drift.md#ldap-has-no-route)
in the companion). Use
