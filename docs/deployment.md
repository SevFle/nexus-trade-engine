...
| `NEXUS_TRUSTED_PROXIES` | `""` | CSV of proxy IPs/CIDRs (e.g. `10.0.0.0/8`). When the direct peer matches, `X-Forwarded-For` / `X-Real-IP` is used for the per-IP rate-limit **and** WS-auth buckets (REST + WS share the resolver in [`engine/api/ip_utils.py`](../engine/api/ip_utils.py)). Otherwise those headers are ignored and a spoofed header cannot escape a bucket. **Required for accurate per-client rate limiting behind a reverse proxy.** |
